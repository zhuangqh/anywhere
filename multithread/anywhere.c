...
#define THREAD_NUM 8 // default thread number
...
void
get_option(int argc, char **argv, uint32_t *port, char **base_path, int *nthreads)
{
  int i = 1;
  while (i < argc) {
    if (strcmp(argv[i], "-p") == 0) {
      ...
      *base_path = argv[i + 1];
      ...
      show_access_log = 1;
    } else if (strcmp(argv[i], "-t") == 0) { // thread number
